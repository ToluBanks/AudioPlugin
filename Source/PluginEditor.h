...
struct FFTDataGenerator
{
    void produceFFTDataForRendering(const juce::AudioBuffer<float>& audioData, const float negativeInfinity) 
    {
        ...
        // first appply windowing funtion to our data
    ...
    void generatePath(const std::vector<float>& renderData, juce::Rectangle<float> fftBounds, int fftSize, float binWidth, float negativeInfinity) 
    {
        ...
        for (int binNum = 1; binNum < numBins; binNum += pathResolution)
        {
            ...
            if (!std::isnan(y) && !std::isinf(y))
            {
                ...
                auto normalizedBinX = juce::mapFromLog10(binFreq, 1.f, 2000.f);
...
// creating the structure for th custom rotary knobs on the custom Gui
