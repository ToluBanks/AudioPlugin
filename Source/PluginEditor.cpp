...
void RotarySliderWithLabels::paint(juce::Graphics& g)
{
    ...
    */
    ...
    //getting center and radius of slider bounsd

    auto center = sliderBounds.toFloat().getCentre();
    auto radius = sliderBounds.getWidth() * 0.5f;

    g.setColour(Colour(0xff800080));
...
void ResponseCurveComponent::timerCallback() 
{
    ...
    // if there are fft buffers to pull, and if we can pull a buffer, generate a path.

    const auto fftBounds = getAnalysisArea().toFloat();
    const auto fftSize = leftChannelFFTDataGenerator.getFFTSize();

    // sample rate / fft size is bin width
    ...
    // while there are paths that can be pilled, pull all many as we can, only display the most recent path
...
{
    // grab coefficeints, update monochain and then repaint.
...
void ResponseCurveComponent::paint(juce::Graphics& g)
{
    ...
    // converting vector of magnitudues into a path and then draw the paths.
    ...
    {
        // -24to24 because the peak response filter has the same range and we want the window to have the same.
        return jmap(input, -24.0, 24.0, outputMin, outputMax);
    };

    // starting a new sub path
...
void ResponseCurveComponent::resized()
{
    ...
    for (auto gDb : gain)
    {
        ...
        g.setColour(gDb == 0.f ? Colour(0xff800080) : Colours::lightslategrey);
...
SimpleEQAudioProcessorEditor::SimpleEQAudioProcessorEditor (SimpleEQAudioProcessor& p)
    ...
    peakQualitySlider(*audioProcessor.apvts.getParameter("Peak Qaulity"), ""), 
...
void SimpleEQAudioProcessorEditor::resized()
{
    ...
    auto bounds = getLocalBounds();
    float hRatio = 26.6 / 100.f;// JUCE_LIVE_CONSTANT(33) / 100.f;
